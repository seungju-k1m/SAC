...
def CargoWOIBatch(self, step, epoch, f):
    ...
    # get the Initial Cell state for LSTM
    ...
    # 1-1. get the reward which includes the log probability

    with torch.no_grad():
        self.agent.actor.setCellState(InitActorCellState)
        nAction, logProb, critics, entropy = \
        ...
        reward -= alpha * logProb
        self.agent.actor.setCellState(InitActorCellState)
        
    # 2. implemented the training using the truncated BPTT
    for _ in range(epoch):
        self.agent.actor.setCellState(InitActorCellState)
        value = self.agent.criticForward(nstate)  # 첫 번째는 유효
        ...
        for i in range(div):
            ...
            f(self, _state, _action, _gT, step, epoch)
            self.agent.actor.detachCellState()
